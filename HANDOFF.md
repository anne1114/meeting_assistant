...
- **Current task:** GitHub Pages live at https://anne1114.github.io/meeting_assistant/ (workflow success, root 200, SPA fallback verified). Testing data was deleted from Supabase; live builds never seed (`VITE_SEED_DISABLED=true`).
- **Status:** Done except ONE user step: deploy the `summarize` Edge Function + add `GEMINI_API_KEY` secret (Dashboard -> Edge Functions -> New Function `summarize` -> paste `supabase/functions/summarize/index.ts` -> Deploy -> Secrets). Until then AI buttons show an error toast.
- **Incoming agent should:**
  1. Verify AI end-to-end once the user deploys the function: open the live site -> New Meeting -> paste transcript -> "Summarize with AI" -> Notes gets filled; Output Review -> Minutes tab -> "AI-refine".
